...
def plot_coordinates(true_data: np.ndarray[..., 3], pred_data: np.ndarray[..., 3], total_time: int = 5,
    ...
    for i, (data, label, color) in enumerate(zip([true_data, pred_data], ['Observed', 'Predicted'], ['blue', 'red'])):
        for dim in range(num_dimensions):
            ...
            axs[dim].plot(time_values, data[:, dim], label=f'{label} trajectories {dim + 1}', color=color)

        axs[dim].set_xlabel('Time (min)')

    # Set y-axis label for each coordinate
    for dim, title in enumerate(['X-position', 'Y-position', 'Z-position']):
        axs[dim].set_ylabel(title)

    axs[0].legend(bbox_to_anchor=(1.0, 1.15), loc="upper right", ncol=1)
...
if __name__ == '__main__':
    ...
    motion_data = np.genfromtxt('../data/Motion.csv', delimiter=',', skip_header=1)[-pls_data.shape[0]:, -3:]
